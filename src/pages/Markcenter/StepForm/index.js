...
export default class StepForm extends PureComponent {
  ...
  render() {
    const {
      location,
      children,
      markcenter: { basicInfo, activityStartTime, activityEndTime, timeLength, activityName },
    } = this.props;
      ...
      >
        <Card bordered={false}>
          <Fragment>
            ...
            {pathList.length > 4 &&
            pathList[pathList.length - 4] != 'info' &&
            pathList[pathList.length - 3] != 'info' &&
            pathList[pathList.length - 2] != 'info' ? (
              <div className={styles.remindModal}>
                ...
                {activityName ? (
                  <div>
                    活动名称：<span style={{ color: '#000' }}>{activityName}</span>
                  </div>
                ) : (
                  <Spin size="large" />
                )}
